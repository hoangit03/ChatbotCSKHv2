    ...
    async def __call__(self, state: AgentState) -> AgentState:
        ...
        if not state.get("query_embedding"):
            ...
            if isinstance(rag_tool, RAGTool):
                ...
                if query:
                    # Thử _embedder trước (v2), fallback _embed (v1)
                    embedder = getattr(rag_tool, "_embedder", None) or getattr(rag_tool, "_embed", None)
                    if embedder:
                        state["query_embedding"] = await embedder.embed_one(query)
